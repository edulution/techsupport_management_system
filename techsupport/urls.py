...
def group_required(*group_names):
    ...
    def in_groups(user):
        if user.is_authenticated:
            return bool(
                set(group_names) & set(user_groups.get(user.groups.first().name, []))
            )
        return False
...
urlpatterns = [
    ...
    path(
        "home/manager/",
        group_required("manager")(ManagerHomePageView.as_view()),
        name="home_manager",
    ),
    path(
        "home/technician/",
        group_required("technician")(TechnicianHomePageView.as_view()),
        name="home_technician",
    ),
    path(
        "home/admin/",
        group_required("admin")(AdminHomePageView.as_view()),
        name="home_admin",
    ),
    path("base/", login_required(base), name="base"),
    path("create-ticket/", login_required(create_ticket), name="create_ticket"),
    path(
        "ticket-detail/user/<int:pk>/",
        login_required(ticket_detail_user),
        name="ticket_detail_user",
    ),
    path(
        "ticket-detail/technician/<int:pk>/",
        login_required(ticket_detail_technician),
        name="ticket_detail_technician",
    ),
    path(
        "manager-dashboard/",
        group_required("manager")(manager_dashboard),
        name="manager_dashboard",
    ),
    path(
        "admin-dashboard/",
        group_required("admin")(admin_dashboard),
        name="admin_dashboard",
    ),
    path("ticket-list/", login_required(ticket_list), name="ticket_list"),
    path(
        "update-ticket/<int:pk>/", login_required(update_ticket), name="update_ticket"
    ),
    path(
        "edit-ticket/<int:pk>/",
        group_required("technician")(edit_ticket),
        name="edit_ticket",
    ),
