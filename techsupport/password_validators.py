...
class MinMaxCharactersValidator(BasePasswordValidator):
    def __init__(self, min_length=5, max_length=8):
        ...
        error_message = _("Password must be between %(min)d and %(max)d characters long.")
...
class DisallowedCharactersValidator(BasePasswordValidator):
    def __init__(self, disallowed_chars=r'[!@#$%^&*()_+={}[\]:;"\'<>,.?/\\|]'):
        ...
        error_message = _("The password contains characters that are not allowed.")
...
class PasswordStrengthValidator(BasePasswordValidator):
    def __init__(self):
        ...
        error_message = _("Password must include at least one uppercase letter, one lowercase letter, and one digit.")
